...
/**
 * Materialert renders Yii's session flash messages.
 *
 ...
 * @package widgets
 */
class Materialert extends BaseWidget
{
    /**
        ...
        foreach ($flashes as $type => $data) {
            if (isset($this->alertLevels[$type])) {
                ...
                foreach ($data as $i => $message) {
                    ...
                    $this->options['class'] = 'materialert ' . $this->alertLevels[$type] . $appendCss;

                    /* assign unique id to each alert box */
                    $this->options['id'] = $this->getId() . '-' . $type . '-' . $i;

                    echo $this->renderHtml($message, $type, $this->options);
                }
     ...
     * @return string
     */
    private function renderHtml($message, $icon, $options = [])
    {
        $html = Html::beginTag('div', $options);
        $html .= "<i class='material-icons'>{$this->icons[$icon]}</i>";
        $html .= $message;
